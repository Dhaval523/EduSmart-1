...
export const ModuleVideoPlayer = ({ url, onEnded }) => {
    ...
    if (!embedUrl) {
      return (
        <div className="text-center text-slate-400">
          ...
          <p className="text-sm text-slate-500 mt-2">Invalid YouTube link</p>
        </div>
      );
    }
    return (
      <iframe
        className="h-full w-full rounded-2xl border border-white/10 shadow-[0_20px_80px_rgba(0,0,0,0.5)]"
        src={embedUrl}
        title="Module video"
  ...
  return (
    <div className="text-center text-slate-400">
      <PlayCircle className="w-20 h-20 mx-auto mb-4 opacity-60" />
      <p className="text-lg font-semibold">Video unavailable</p>
      <p className="text-sm text-slate-500 mt-2">Unsupported or missing video link</p>
    </div>
  );
};
