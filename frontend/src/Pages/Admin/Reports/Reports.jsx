        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {revenue.isLoading ? <><ReportCardSkeleton /><ReportCardSkeleton /></> : trend.length === 0 ? <ReportEmpty /> : <>
              <div className="card"><h2 className="text-lg font-bold text-[#0f172a]">Revenue Over Time</h2><div className="h-80 mt-4"><ResponsiveContainer width="100%" height="100%"><LineChart data={trend}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip formatter={(value) => [formatINR(value), "Revenue"]} /><Line dataKey="amount" type="monotone" stroke="#0ea5a4" strokeWidth={3} dot={false} /></LineChart></ResponsiveContainer></div></div>
              <div className="card"><h2 className="text-lg font-bold text-[#0f172a]">Revenue by Course</h2><div className="h-80 mt-4"><ResponsiveContainer width="100%" height="100%"><BarChart data={rows.slice(0, 10)}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3" /><XAxis dataKey="courseName" hide /><YAxis /><Tooltip formatter={(value) => [formatINR(value), "Revenue"]} /><Bar dataKey="revenue" fill="#0f766e" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></div></div>
            </>}
          </div>
    ...
    if (activeTab === "users") {
      ...
      return (
        <div className="space-y-6">
          ...
          {users.isLoading ? <ReportCardSkeleton /> : <div className="card"><h2 className="text-lg font-bold text-[#0f172a]">New Users Growth</h2><div className="h-80 mt-4"><ResponsiveContainer width="100%" height="100%"><LineChart data={rows}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip /><Line dataKey="count" type="monotone" stroke="#0ea5a4" strokeWidth={3} dot={false} /></LineChart></ResponsiveContainer></div></div>}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {enrollments.isLoading ? <><ReportCardSkeleton /><ReportCardSkeleton /></> : trendRows.length === 0 ? <ReportEmpty /> : <>
              <div className="card"><h2 className="text-lg font-bold text-[#0f172a]">Enrollments Over Time</h2><div className="h-80 mt-4"><ResponsiveContainer width="100%" height="100%"><LineChart data={trendRows}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip /><Line dataKey="count" type="monotone" stroke="#0ea5a4" strokeWidth={3} dot={false} /></LineChart></ResponsiveContainer></div></div>
              <div className="card"><h2 className="text-lg font-bold text-[#0f172a]">Enrollments by Course</h2><div className="h-80 mt-4"><ResponsiveContainer width="100%" height="100%"><BarChart data={courseRows.slice(0, 10)}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3" /><XAxis dataKey="courseName" hide /><YAxis /><Tooltip /><Bar dataKey="count" fill="#0f766e" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></div></div>
            </>}
          </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {engagement.isLoading ? <><ReportCardSkeleton /><ReportCardSkeleton /></> : <>
            <div className="card"><h2 className="text-lg font-bold text-[#0f172a]">Daily Active Users</h2><div className="h-80 mt-4"><ResponsiveContainer width="100%" height="100%"><LineChart data={dauRows}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip /><Line dataKey="count" type="monotone" stroke="#0ea5a4" strokeWidth={3} dot={false} /></LineChart></ResponsiveContainer></div></div>
            <div className="card"><h2 className="text-lg font-bold text-[#0f172a]">Lessons Completed</h2><div className="h-80 mt-4"><ResponsiveContainer width="100%" height="100%"><BarChart data={lessonRows}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip /><Bar dataKey="count" fill="#0f766e" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></div></div>
          </>}
        </div>
  ...
  return (
    <div className="page-bg py-8">
      <div className="page-shell space-y-6">
        <div className="card">
          <h1 className="text-3xl font-black text-[#0f172a]">Reports</h1>
          <p className="text-sm text-[#51607b] mt-1">Switch between report categories without leaving this page.</p>
            ...
            {TABS.map((tab) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-semibold transition ${
                  activeTab === tab.key ? "bg-[#0ea5a4] text-white" : "bg-[#f5f7fb] text-[#0f172a] hover:bg-[#e7f5f4]"
                }`}
              >
                {tab.label}
              </button>
